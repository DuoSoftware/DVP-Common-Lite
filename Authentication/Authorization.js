module.exports = function (options) {
  ...
  var middleware = function (req, res, next) {
    ...
    let scopes = req.user.scope || req.scope.scope;

    if (scopes) {
      var globalselector = scopes.filter(function (item) {
      ...
      if (globalselector && globalselector.length > 0) {
        ...
        if (resource) {
          var selected = scopes.filter(function (item) {
